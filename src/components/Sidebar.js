// import icons
import { FaBookMedical, FaNotesMedical, FaArrowRight } from "react-icons/fa";
...
export default function Sidebar() {
  ...
  return(
    <div className={`text-sm md:text-lg ${responsive ? `w-[60px]` : 'w-[250px]'} md:w-[250px] text-white bg-[#089bab] h-screen flex flex-col items-center p-5 font-bold fixed z-10 transition-all duration-300`}>
      <button onClick={() => setResponsive(!responsive)} className="md:hidden absolute right-[-15px] top-[20px] text-white bg-[#089bab] w-[20px] h-[50px] rounded-br-lg rounded-tr-lg flex justify-end items-center px-1 "><FaArrowRight className="text-sm" /></button>
      <div className={`flex flex-col items-center md:flex mb-2 ${responsive ? 'hidden' : 'flex'}`}>
        ...
        <h1 className="text-[16px] md:text-[20px] font-extrabold mt-5">Zaid Alshamaa</h1>

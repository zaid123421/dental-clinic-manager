...
export default function TreatmentsPlans() {
  ...
  const showCategories = allCategories?.map((category, index) => (
    <button
      ...
      className={`${
        category.name === selectedType ? "bg-[#089bab] text-white" : ""
      } py-2 px-3 rounded-xl w-full lg:w-fit font-semibold hover:text-white hover:bg-[#089bab] duration-300`}
      key={index}
    >
      {category.name}
    </button>
  ...
  const showPlans = filteredPlans?.map((plan, index) => (
    ...
    >
      <p className=" flex justify-between">
        Name: <span className="ml-2 text-[#089bab]">{plan.name}</span>
      </p>
      <p className="flex justify-between">
        Category:{" "}
        <span className="ml-2 text-[#089bab]">{plan.category.name}</span>
      </p>
      <p className="flex justify-between">
        Cost:{" "}
        <span className="ml-2 text-[#089bab]">
          {plan.cost.toLocaleString()}
        </span>
      </p>
      <p className="flex justify-between">
        Tooth Status After Plan:{" "}
        <span className="ml-2 text-[#089bab]">{plan.tooth_status?.name}</span>
      </p>
      <div className="flex text-2xl justify-center mt-2">
        <div
          onClick={() => nav(`/treatment-plan?id=${plan.id}`)}
      ...
      <div className="page-content p-3 md:py-5 md:p-5 bg-[#089bab1c] overflow-hidden">
        ...
        <div className="mt-3 flex flex-col lg:flex-row">
          <Button
            ...
            className="min-w-[250px] hidden md:flex"
            variant="primary"
        ...
        <div className="flex items-center">
          <p className="text-3xl font-semibold ml-1 mr-4">
          ...
          <div className={`${selectedType === "All" ? "hidden" : "flex"}`}>
            <MdDelete
              ...
              className="text-xl text-red-500 md:text-2xl mr-2 hover:text-red-600 duration-300 cursor-pointer"
            />
            <MdEdit
              onClick={() => {
